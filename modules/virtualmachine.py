...
"""
Virtual machine specific handeling for Netbox to Zabbix
"""
from os import sys
...
class VirtualMachine():
    ...
    def set_hostgroup(self, hg_format, nb_site_groups, nb_regions):
        ...
        manufacturer = None # self.nb.device_type.manufacturer.name
    ...
    def get_templates_cf(self):
        ...
        # Check if platform is None
        if self.nb.platform is None:
            raise TemplateError("Platform is None for this device.")

        # Get Zabbix templates from the device type
        device_type_cfs = self.nb.platform.custom_fields
        ...
        if template_cf in device_type_cfs:
            ...
            e = (f"Custom field {template_cf} not "
                f"found for {self.nb.platform.manufacturer.name}"
                f" - {self.nb.platform.display}.")
    ...
    def promoteMasterDevice(self):
        ...
        if masterid == self.id:
            self.logger.debug(f"Device {self.name} is primary cluster member. "
                              ...
                              f"{self.nb.cluster.name}.")
            self.name = self.nb.cluster.name
            return True
